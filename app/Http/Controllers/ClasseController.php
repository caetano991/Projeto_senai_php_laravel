...
class ClasseController extends Controller
{
    ...
    public function create(Course $course){
        ...
        return view('classes.create', compact('course'));
    ...
    // 1. Recuperar a ultima ordem da aula do curso
    $lastOrderClasse = Classe::where('course_id', $request->course_id)
        ->orderBy('order_classe', 'DESC')
        ->first();

    // 2. AQUI ENTRA A LINHA DE CÓDIGO (Proteção contra valor nulo)
    $nextOrder = $lastOrderClasse ? $lastOrderClasse->order_classe + 1 : 1;

    // 3. Cadastrar no banco de dados na tabela aula
    Classe::create([
        'name'         => $request->name,
        'description'  => $request->description,
        'order_classe' => $nextOrder, // <--- Trocamos o cálculo direto pela variável protegida
